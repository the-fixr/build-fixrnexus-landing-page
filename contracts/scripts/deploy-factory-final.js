...
async function main() {
  console.log("Deploying fixed OracleFactory (recompiled with patched FarcasterOracle)...");
  ...
  // Load NEW compiled contract (with fixed FarcasterOracle)
  ...
  const deployTx = await factory.getDeployTransaction(REGISTRY);
  const gasEstimate = await provider.estimateGas(deployTx);
  console.log("Estimated gas:", gasEstimate.toString());

  console.log("\nDeploying with 20% buffer...");
  ...
  console.log("\nSetting validators...");
  const tx = await contract.setValidators(validators);
  await tx.wait();
  ...
  console.log("\nOld factory: 0xBe17f562BF4068fb429D7729D0EAa64cEBf3ed88");
