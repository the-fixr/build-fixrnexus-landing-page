...
export function AuthGuard({ children }: { children: ReactNode }) {
  ...
  const isAuthorized = address ? AUTHORIZED_WALLETS.includes(address.toLowerCase()) : false;
      ...
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md w-full">
          <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl p-8 text-center">
            ...
            <p className="text-gray-400 text-sm mb-8">Connect your wallet to access the admin portal. Only authorized wallets can access this area.</p>
            {isConnected && !isAuthorized && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
            ...
            <p className="text-xs text-gray-600 mt-6">Supports MetaMask, Coinbase Wallet, WalletConnect & more</p>
          </div>
        </motion.div>
      </div>
    );
