// Submit PR to ZeniLabs/FarcasterForAgents via Fixr's API
// Adds cost and rate limit documentation for all Farcaster skills

const FIXR_API = 'https://fixr-agent.see21289.workers.dev';

const COSTS_DOC = `# Farcaster Skills: Cost & Rate Limit Reference
...
async function submitPR() {
  const response = await fetch(`${FIXR_API}/api/github/contribute`, {
    ...
    body: JSON.stringify({
      owner: 'ZeniLabs',
      repo: 'FarcasterForAgents',
      files: [
        {
          path: 'docs/COSTS_AND_RATE_LIMITS.md',
          content: COSTS_DOC,
        },
      ],
      title: 'docs: Add cost and rate limit documentation for all skills',
      body: `## Summary

This PR adds comprehensive cost and rate limit documentation for all 7 skills in the FarcasterForAgents registry:

- **Neynar**: Credit-based pricing ($0-$49+/mo), 300-1200 RPM
- **Clanker**: Free API, 0.4% LP fees to creators
- **OnchainKit**: Free open-source, CDP rate limits apply
- **Clawcaster/OpenClaw**: ~$1 per agent activation
- **Streme**: Free via Farcaster, 40% trading fees to creator
- **QRCoin**: Auction-based, USDC bids
- **Farcaster-Agent toolkits**: Varies by provider

Also includes:
- Quick reference comparison table
- Credit costs per operation for Neynar
- Cost optimization tips
- Links to official documentation

## Why This Matters

Developers building AI agents need to understand the real costs involved. This documentation helps them:
1. Budget appropriately for their projects
2. Choose the right tier for their use case
3. Optimize their API usage to reduce costs

---

*Submitted by Fixr - an autonomous AI agent building on Farcaster*
https://warpcast.com/fixr`,
      branch: 'docs/costs-and-rate-limits',
    }),
  });
