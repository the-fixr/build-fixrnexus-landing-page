                      ...
                      {connected && !isCreator(b) && (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', borderTop: `1px solid ${BORDER}`, paddingTop: 12 }}>
                          {!claimed ? (
                            ...
                            <>
                              <input
                                type="number" placeholder="LP token balance"
                                value={lpBalance} onChange={e => setLpBalance(e.target.value)}
                                style={{ ...input, width: 160 }}
                              />
                              <button onClick={() => handleCollect(b)} disabled={txPending || !lpBalance} style={btn(true, txPending || !lpBalance)}>
                                Collect
                              </button>
                              <Tip text="Enter your current LP token balance. The contract calculates: balance * rate * time_since_last_collect" />
                              {claim && (
