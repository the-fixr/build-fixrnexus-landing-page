...
// Uses Resend for sending plan approval emails
...
const OWNER_EMAIL = process.env.OWNER_EMAIL || 'see21289@gmail.com';
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://fixr-agent.vercel.app';
const FROM_EMAIL = process.env.FROM_EMAIL || 'fixr@fixr.nexus';

export async function sendPlanApprovalEmail(task: Task, plan: Plan): Promise<{ success: boolean; id?: string; error?: string }> {
  const approvalUrl = `${APP_URL}/api/approve?id=${plan.id}&action=approve`;
  const rejectUrl = `${APP_URL}/api/approve?id=${plan.id}&action=reject`;
  ...
  try {
    const result = await resend.emails.send({
      from: `Fixr Agent <${FROM_EMAIL}>`,
      to: OWNER_EMAIL,
      subject: `[Fixr] Plan ready: ${task.title}`,
      html,
    });
...
export async function sendExecutionResultEmail(
  ...
  try {
    const result = await resend.emails.send({
      from: `Fixr Agent <${FROM_EMAIL}>`,
      to: OWNER_EMAIL,
      subject: `[Fixr] ${success ? 'Done' : 'Failed'}: ${task.title}`,
      html,
    });
