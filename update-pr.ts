// Update existing PR with corrected Clanker fee info
...
async function updatePR() {
  // First get Fixr's GitHub username
  const userRes = await fetch(`${FIXR_API}/api/github/user`);
  const userData = await userRes.json() as { success: boolean; user?: { login: string } };
  const fixrUsername = userData.user?.login;

  if (!fixrUsername) {
    console.log('Could not get Fixr GitHub username');
    return;
  }

  console.log(`Pushing to ${fixrUsername}/FarcasterForAgents branch docs/costs-and-rate-limits`);
  ...
  const response = await fetch(`${FIXR_API}/api/github/push`, {
    ...
    body: JSON.stringify({
      ...
      message: 'fix: Correct Clanker fee split to 80/20 (80% to creators)',
    }),
  });
